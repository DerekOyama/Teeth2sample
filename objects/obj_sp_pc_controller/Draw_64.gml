...
for(var i = 0; i < 5; i++)
{
	...
	for(var i = 0; i < 5; i++)
	{
		if(global.unit_selection[i] != -1)
		{
			...
			draw_sprite_stretched(spr_cd_black, 0, 60*i + 28, 168, 64, 64 * (unit_current_cd[i] / unit_real_stats[i][ulib.cd]))
		}
	}
	...
	for(var i = 0; i < 5; i++)
	{
		if(global.unit_selection[i] != -1)
		{
			draw_text(60 * i + 32, 170, unit_real_stats[i][ulib.cost])
		}
	}
	...
	if(draw_desc_num >= 5)
	{
		...
		if(global.spovid.saved_items[draw_desc_num - 5] != -1)
		{
			draw_sprite_ext(global.ilore[ilorelib.statsprite][global.spovid.items_owned[global.spovid.saved_items[draw_desc_num - 5]][ilib.stat]], global.spovid.items_owned[global.spovid.saved_items[draw_desc_num - 5]][ilib.tier], tmx + 150, tmy + 10, 0.5, 0.5, 0, c_white, 1);
		}
	...
	draw_text(82, 138, player_money_generation_rate)
